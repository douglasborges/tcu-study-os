(() => {
  ...
  function canonicalizePortuguese_(clean) {
    ...
    if (!portuguese) {
      portuguese = { id: 'portugues', name: 'Português', active: true, mode: 'Teoria', order: 1, frequency: 2, priority: 'Alta', manualHours: '', source: '', sourceUrl: '', notes: 'Criada pela migração v5.' };
  ...
  function ensureDiscipline(name, mode = 'Teoria') {
    ...
    if (!topicsForDiscipline(disc).length) state.topics.push({ id: uid('topic'), disciplineId: disc.id, disciplineName: disc.name, title: 'Assunto 1', details: 'Edite este tópico em Conteúdo.', status: 'Em espera', priority: disc.priority, order: 1, notes: '', sourceUrl: '' });
    saveState();
    return disc;
  }
  ...
  function cycleArray() {
    ...
    base.forEach(d => {
      ...
      for (let extra = 1; extra < n; extra += 1) {
        const insertAt = Math.min(arr.length, Math.max(1, Math.round((extra * arr.length) / n)));
        arr.splice(insertAt, 0, d);
      }
    });
    return arr;
  }
  ...
  function renderCyclePizza() {
    ...
    if (!items.length) return '<div class="card"><h3>Ciclo em pizza</h3><p class="empty">Ative disciplinas no Ciclo para montar o gráfico.</p></div>';
    let acc = 0;
    ...
    // Garante fechamento visual, mesmo com arredondamento dos percentuais.
    if (acc < 100) segments.push(`${items[items.length - 1].color} ${acc}% 100%`);
    return `
      <div class="card cycle-pizza-card">
        <div class="section-title compact"><h3>Ciclo em pizza</h3><span class="badge dark">${cycleArray().length} etapa(s)</span></div>
    ...
    const content = `
      ...
      <section class="card">
        ...
        <form id="register-form" class="form-grid" style="margin-top:18px">
          ...
          <div class="field span-5"><label>Tópico cadastrado</label><select id="reg-topic"><option value="">Selecione a disciplina para carregar tópicos...</option></select></div>
